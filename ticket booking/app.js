...
createApp({
    ...
    methods: {
        handleClick(key) {
            let clickedSeat = this.seats[key];
            ...
            if (clickedSeat.type == "available" && this.selectedSeat.length > 2) {
                alert('You cant select more than 3 seats at a time');
                return;
            }
    ...
    watch: {
        couponCode(newValue){
            if (this.couponCode.length == 10) {
                let searchCopun = this.coupons.filter(coupon => coupon.code  == newValue);


                if (searchCopun.length == 1) {
                    this.appliedCoupon = searchCopun[0]; 
                    this.couponCode = ""
                } else{
